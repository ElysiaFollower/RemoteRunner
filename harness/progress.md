...
## F-032 Register Existing tmux（passing，待切换）
...
- 完整门禁 40 passed；0.5.0 wheel 非 editable 安装冒烟通过。任务合同已归档到
  `plans/archive/2026-07-22-register-existing-tmux.md`。
- main、系统 editable runtime、默认 state/default tmux 和 global Skill 保持不变。
...
Session 已 destroy/purge，新默认 state 当前为空。F-030、F-031、F-032 均为 passing；F-032
仍在隔离分支等待显式切换，当前无 active WIP。
