...
- 下一步最佳动作：人工完成新建 manual-auth replacement session 的登录，然后只用 V2 `send/read/interrupt` 恢复远端工作。
- 2026-07-15：在运行任务停止后的安全窗口，将已验证的 OpenSSH PTY 基线和 Terminal Session V2 提交链快进合入 `main`；editable install 无需重装即加载主 worktree 新实现，global live skill 已同步。缺失 pane 的旧 record 通过 liveness 刷新为 `lost`，另建 V2 replacement session 停在人工认证边界。为排除测试误杀，使用独立 sentinel 分别复跑真实 tmux 聚焦测试与完整 pytest，两次 sentinel 均存活；未向远端发送业务命令。
...
- 2026-07-08：修正真实使用暴露的 Remote Runner `session exec` 语义漂移。结论：`session exec` 是向同一持久 shell 输入命令并捕获结构化证据，不是隔离 batch/job runner；需要 `exit "$rc"`、上传输入、回收产物、销毁临时上下文时走 `run once` 或后续 job/batch 能力。实现：Linux/tmux wrapper 去掉 `|| exit "$__rr_rc"` 兜底，只 `return` 回持久 shell；`RemoteSessionManager.exec()` 捕获 `BaseException` 以记录 KeyboardInterrupt 并清 busy。同步 REQUIREMENTS、API、README、getting-started、overview、MVP spec、feature list 和 handoff。验证：聚焦测试 3 passed；`tests/test_remote_runner_mvp.py` 53 passed；harness-check 0 warnings；init 通过。
- 2026-07-07：通过 `a100-work` 长期真实 session 在 91_A100 上完成第一轮 full-duplex/dialogue-synthesis 竞品审计。远端总表写入 `/home/lujingyu/project/ljm/FULL_DUPLEX_COMPETITOR_AUDIT_SUMMARY.md`。已实际跑通/抽样：dGSLM autonomous stereo samples、Behavior-SD public two-channel dataset sample、FireRedTTS-2 dialogue TTS、Dia dialogue TTS、ZipVoice stereo dialogue、BayLing-Duplex response；NTPP、CoVoMix、DuplexCascade、Moshi、MOSS-TTSD 分别按公开可复现性和阻塞原因写入各项目 `AUDIT_REPRO.md` 或 docs。
- 2026-07-07：真实可用性补测发现并修复两个边界问题：`openssh-pty` 曾错误接受 `session exec --mode background` 并留下不可追踪 running record；`run once` 曾创建未登录临时 PTY session 后挂起。现在二者都会快速拒绝且不创建 session/command record。残留限制：长 transcript 下 `session read --since` 增量不可靠；大输出截断后本地 command log 也未保留完整输出。
- 2026-07-07：新增 `F-025 Remote Runner OpenSSH PTY backend` active。背景：`91_A100` 类机器无法稳定走 Paramiko exec/SFTP，也不适合端口转发；但 `ssh -tt 91_A100` 在用户输入密码后能进入目标交互 shell。任务范围限定为本机 tmux 托管 `/usr/bin/ssh -tt <alias>`、用户手动 attach 登录、登录后支持 session send/read/wait-mode exec/destroy；不保存密码、不记录真实网关细节、不做文件传输或后台命令。
